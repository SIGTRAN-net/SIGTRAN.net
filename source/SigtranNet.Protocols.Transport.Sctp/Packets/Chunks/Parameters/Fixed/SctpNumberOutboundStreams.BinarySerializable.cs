...
internal readonly partial struct SctpNumberOutboundStreams : IBinarySerializable<SctpNumberOutboundStreams>
{
    /// <inheritdoc />
    public static SctpNumberOutboundStreams FromReadOnlyMemory(ReadOnlyMemory<byte> memory) =>
        new(BinaryPrimitives.ReadUInt16BigEndian(memory.Span));

    /// <inheritdoc />
    public static SctpNumberOutboundStreams Read(BinaryReader reader) =>
        new(BinaryPrimitives.ReadUInt16BigEndian(new ReadOnlyMemory<byte>(reader.ReadBytes(LengthFixed)).Span));

    /// <inheritdoc />
    public static SctpNumberOutboundStreams Read(Stream stream)
    {
    ...
    /// <inheritdoc />
    public static async Task<SctpNumberOutboundStreams> ReadAsync(Stream stream, CancellationToken cancellationToken = default)
    {
        var memory = new Memory<byte>(new byte[LengthFixed]);
